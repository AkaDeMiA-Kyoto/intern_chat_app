...
def create_users(n):
    ...
    my_id = CustomUser.objects.get(username="tomoya").id
    ...
    for _ in range(len(user_ids)):
        sent_talk = Message(
            send_user=CustomUser.objects.get(id=my_id),
            receive_user=CustomUser.objects.get(id=random.choice(user_ids)),
            message=fakegen.text(max_nb_chars=50),
        )
        received_talk = Message(
            send_user=CustomUser.objects.get(id=random.choice(user_ids)),
            receive_user=CustomUser.objects.get(id=my_id),
            message=fakegen.text(max_nb_chars=50),
        )
        talks.extend([sent_talk, received_talk])
    Message.objects.bulk_create(talks, ignore_conflicts=True)

    # Message の time フィールドは auto_now_add が指定されているため、 bulk_create をするときに
    # time フィールドが自動的に現在の時刻に設定されてしまいます。
    # 最新の 2 * len(user_ids) 個分は先ほど作成した Message なので、これらを改めて取得し、
