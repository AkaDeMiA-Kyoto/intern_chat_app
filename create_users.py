...
def create_users(n):
    ...
    my_id = CustomUser.objects.get(username="仙波").id
    ...
    for _ in range(len(user_ids)):
        sent_talk = Messages(
            sender_name=CustomUser.objects.get(id=my_id),  
            receiver_name=CustomUser.objects.get(id=random.choice(user_ids)),
            description=fakegen.text(),
        )
        received_talk = Messages(
            sender_name = CustomUser.objects.get(id=random.choice(user_ids)),
            receiver_name=CustomUser.objects.get(id=my_id),
            description=fakegen.text(),
        )
    ...
    talks = Messages.objects.order_by("-time")[: 2 * len(user_ids)]
    for talk in talks:
