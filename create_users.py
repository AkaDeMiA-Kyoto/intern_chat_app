...
def create_users(n):
    ...
    talks = Talk.objects.order_by("-talk_at")[: 2 * len(user_ids)]
    for talk in talks:
        talk.talk_at = fakegen.date_time_this_year(tzinfo=tz.gettz("Asia/Tokyo"))
    Talk.objects.bulk_update(talks, fields=["talk_at"])
